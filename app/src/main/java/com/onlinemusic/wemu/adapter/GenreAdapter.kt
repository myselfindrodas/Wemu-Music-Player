...
class GenreAdapter (var context: Context

) :
    RecyclerView.Adapter<GenreAdapter.MyViewHolder>()  {
    var sessionManager: SessionManager? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val v: View = LayoutInflater.from(context).inflate(R.layout.genreslayout, parent, false)
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.title.text = modelList[position].cateogryName



        Glide.with(context)
            .load(modelList[position].backgroundThumb)
            .timeout(6000)
