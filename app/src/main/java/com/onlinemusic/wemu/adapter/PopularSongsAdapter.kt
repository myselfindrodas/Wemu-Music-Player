...
class PopularSongsAdapter (var context: Context,
                           var onItemClickListener: OnItemClickListener
) :
    RecyclerView.Adapter<PopularSongsAdapter.MyViewHolder>()  {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        Glide.with(context)
            .load(modelList?.get(position)?.thumbnail)
            .timeout(6000)
        ...
        holder.imgTitle.text = modelList?.get(position)?.title

        if (MainActivity.songId==modelList?.get(position)!!.song_id){
